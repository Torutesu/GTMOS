...
計画には無かったが、**出力を人間の目で見た**ことでしか見つからなかったもの。
`pnpm accept` は緑のまま、これらは全部起きていた。

| 症状 | 原因 | 対応 |
| --- | --- | --- |
| デモの中に `sdv-placeholder-vite-workspace-name` が本文として表示される | プレースホルダを機械的な文字列で埋めていた。env の値は**画面に出る**ことがある | `WORKSPACE_NAME` 等の表示系キーは "Northwind" のような自然な値、トークン類は従来どおり露骨なマーカーのまま(フレームに映ったら不具合として見えてほしいので) |
| **デモが閲覧者の OS テーマで見た目を変える** | 再生 iframe 内で `prefers-color-scheme` が**閲覧者の**設定に対して評価される。撮影時に light だったアプリが、dark の閲覧者には dark で再生される | **未修正。** 動画とデモが食い違う = このプロダクトの中核の主張が崩れる。キャプチャ時のスキームを記録し、`snap.styles` の `@media (prefers-color-scheme)` ブロックを**平坦化**する(一致する側を無条件化し、他方を削除)必要がある |
| デモがサーバー無しでは開けない | `demo/` はディレクトリ。渡す手段が「ホスティングして」しかなかった | `sdv export <run>` を追加。全アセットを data URI 化した 1 ファイル HTML(実測 1.2MB)。`file://` から動作し、**外部リクエスト 0 本**を受け入れテストで検証 |

### まだ返していない負債

- **自分で書いていないリポジトリに対して一度も動かしていない。** Taskloop はルートが綺麗で認証状態がコミット済みで spec が role セレクタを使う、こちらが作った最良ケース。「リポジトリを指すだけ」という主張は未検証
- 再生時のカラースキーム漂流(上表)
